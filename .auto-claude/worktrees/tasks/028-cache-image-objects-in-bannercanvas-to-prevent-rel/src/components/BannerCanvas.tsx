...
const BannerCanvasComponent = forwardRef<BannerCanvasHandle, BannerCanvasProps>(
  (
    ...
    // Helper function to get or create cached Image objects
    const getOrCreateImage = (url: string): HTMLImageElement => {
      const cached = imageCache.current.get(url);
      if (cached) {
        if (import.meta.env.DEV) {
          console.log('[BannerCanvas Cache] HIT:', url.substring(0, 50) + '...');
        }
        return cached;
      }

      if (import.meta.env.DEV) {
        console.log('[BannerCanvas Cache] MISS - Creating new Image:', url.substring(0, 50) + '...');
      }
      const img = new Image();
      imageCache.current.set(url, img);
      img.src = url; // Set src after caching to trigger load
      return img;
    };

    // Helper function to prune unused images from cache
    const pruneImageCache = () => {
      const usedUrls = new Set<string>();

      // Collect background image URL
      if (backgroundImage) {
        usedUrls.add(backgroundImage);
      }

      // Collect profile pic URL
      if (profilePic) {
        usedUrls.add(profilePic);
      }
      ...
      // Remove cache entries not in current use
      const cachedUrls = Array.from(imageCache.current.keys());
      const beforeCount = cachedUrls.length;
      let removedCount = 0;
      ...
      if (removedCount > 0) {
        if (import.meta.env.DEV) {
          console.log(`[BannerCanvas Cache] Pruned ${removedCount} unused image(s). Cache: ${beforeCount} → ${beforeCount - removedCount}`);
        }
      }
    };
    ...
    // Cleanup image cache on unmount to prevent memory leaks
    useEffect(() => {
      return () => {
    ...
    // Prune unused images from cache when elements or images change
    useEffect(() => {
      pruneImageCache();
    }, [backgroundImage, elements, profilePic]);
    ...
    const renderCanvas = (
      ...
      if (backgroundImage) {
        const img = getOrCreateImage(backgroundImage);
        if (img.complete) {
    ...
    const drawElementsAndOverlays = (
      ...
      elements.forEach((el) => {
        ...
        if (el.type === 'text') {
          ...
          const img = getOrCreateImage(el.content);
          if (img.complete) {
