...
const AppContent = () => {
  ...
  // Memoize keyboard shortcut handlers with useCallback to prevent unnecessary event listener
  // re-registration on every render. Without memoization, inline functions create new references
  // on every render, causing useKeyboardShortcuts hook to remove and re-add DOM event listeners
  // unnecessarily, leading to memory churn and performance degradation.

  // onGenerate: Ctrl+G - Validate prompt and trigger image generation
  // Dependencies: genPrompt (validate before generation), announce (error messages), handleGenerate (action)
  const onGenerate = useCallback(() => {
    ...
    handleGenerate();
  }, [genPrompt, announce, handleGenerate]);

  // onToggleHistory: Ctrl+H - Toggle chat history panel visibility
  // Dependencies: showChatHistory (for announcement text), announce (screen reader notification)
  const onToggleHistory = useCallback(() => {
    setShowChatHistory((prev) => !prev);
    announce(showChatHistory ? 'Chat history closed' : 'Chat history opened', 'polite');
  }, [showChatHistory, announce]);

  // onClosePanels: Escape - Close all panels (history and settings)
  // Dependencies: [] (empty - only uses stable React setState functions)
  // Note: This handler never re-creates, providing optimal memoization
  const onClosePanels = useCallback(() => {
    setShowChatHistory(false);
    setShowSettings(false);
  }, []);

  // onOpenSettings: Ctrl+, - Open settings modal
  // Dependencies: announce (screen reader notification)
  const onOpenSettings = useCallback(() => {
    setShowSettings(true);
    announce('Settings opened', 'polite');
  }, [announce]);

  // Memoize shortcuts array to prevent event listener re-registration on every render.
  // The array only changes when one of the handler functions actually changes (due to their dependencies).
  // This ensures useKeyboardShortcuts only re-registers listeners when handler logic truly needs updating.
  const shortcuts = useMemo(() => getDefaultShortcuts({
    onGenerate,
    onToggleHistory,
    onClosePanels,
    onOpenSettings,
  }), [onGenerate, onToggleHistory, onClosePanels, onOpenSettings]);
