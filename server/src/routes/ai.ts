...
} from '../prompts/promptEnhancer';
import { aiRateLimit } from '../lib/rateLimit';
// TEMPORARILY DISABLED: Credit service imports commented out while credit checks are bypassed
// import { checkCredits, deductCredits, type OperationType } from '../services/creditService';
import { type OperationType } from '../services/creditService';
...
const handleCredits = async (
  ...
  // ============================================================
  // TEMPORARILY DISABLED: Credit checks bypassed for unlimited usage
  // TODO: Re-enable credit system when ready for production billing
...
const validateMessages = (messages: unknown): void => {
  ...
  if (total > MAX_TOTAL_CHARS)
    throw new Error(`Conversation too long (${total} chars). Max ${MAX_TOTAL_CHARS}.`);
};
...
aiRouter.post('/chat', authMiddleware, async (c) => {
  ...
  try {
    ...
    const response = await traceLLMCall(model, 'openrouter', messages, 'llm.chat_completion', () =>
...
aiRouter.post('/image/edit', authMiddleware, async (c) => {
  try {
    ...
    const ensureDataUri = (img: string) =>
      img.startsWith('http') || img.startsWith('data:') ? img : `data:image/png;base64,${img}`;
    ...
    if (
      ...
      (provider === 'replicate' && model?.includes('flux-fill'))
    ) {
      try {
...
aiRouter.post('/image/generate', authMiddleware, async (c) => {
  ...
  try {
    ...
    if (provider === 'openrouter') {
      console.warn(
        '[Image Gen] OpenRouter does not support image generation. Falling back to Replicate.',
      );
    ...
    if (provider === 'replicate' || provider === 'openrouter' || !provider) {
      ...
      if (model === 'stability-ai/stable-diffusion-3-large')
      ...
      // Imagen Logic - use same dimension handling as Flux
        ...
        if (width && height) {
          ...
          console.log(
            `[Imagen] Using aspect ratio ${input.aspect_ratio} from dimensions ${width}x${height}`,
          );
        } else if (aspect_ratio) {
          input.aspect_ratio = aspect_ratio;
        } else {
      ...
      const output = await traceLLMCall(modelId, 'replicate', prompt, 'llm.image_generate', () =>
...
aiRouter.post('/image/upscale', authMiddleware, async (c) => {
  ...
  try {
    const output = await traceLLMCall(modelToUse, 'replicate', 'upscale', 'llm.image_tool', () =>
...
aiRouter.post('/image/outpaint', authMiddleware, async (c) => {
  ...
  try {
    const output = await traceLLMCall(modelToUse, 'replicate', 'outpaint', 'llm.image_tool', () =>
...
aiRouter.post('/prompt/enhance', authMiddleware, async (c) => {
  const { prompt, context } = (await c.req.json()) as {
    prompt: string;
