...
(function NanoBannaImageGenTest() {
  ...
  const TEST_PROMPTS = {
    ...
    complex:
      'Modern tech startup office with large windows, city skyline view, clean minimalist design with geometric patterns',
    creative:
      'Abstract art with flowing colors in purple, blue and gold, professional aesthetic suitable for LinkedIn',
  };
  ...
  async function testEmptyPrompt() {
    ...
    const notification =
      document.querySelector('[class*="notification"]') || document.querySelector('.fixed.top-20');
  ...
  async function testResolutionSwitching() {
    ...
    for (const res of resolutions) {
      ...
      if (btn) {
        ...
        const isActive =
          btn.className.includes('bg-zinc-700') || btn.className.includes('text-white');
  ...
  async function runTests(options = {}) {
    const { skipGeneration = false, prompt = TEST_PROMPTS.basic, resolution = '1K' } = options;
