...
export const getConversation = async (id: string) => {
  try {
    const { conversation } = await api.get<{ conversation: ChatConversation }>(
...
export const addMessage = async (conversationId: string, message: Partial<ChatMessage>) => {
  try {
    const { message: newTask } = await api.post<{ message: ChatMessage }>(
...
export const getMessages = async (
  conversationId: string,
  _options?: { limit?: number; offset?: number; order?: 'asc' | 'desc' },
) => {
  try {
...
export const saveConversationExport = async (conversationId: string) => {
  ...
  const file = new File([blob], `conversation_${conversationId}.json`, {
