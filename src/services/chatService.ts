...
import { api } from './api';
import { ChatMessage, OpenRouterContentItem, PROFILE_ZONE_CONSTRAINT } from './llm-types';
...
export const generateDesignChatResponse = async (
  ...
  const messages: ChatMessage[] = history.map((h) => ({
    ...
    content: h.parts
      ...
      .filter(Boolean) as OpenRouterContentItem[],
  }));
...
export const generateAgentResponse = async (
  ...
  messages.unshift({
    role: 'system',
    content: 'You are Nano, an expert design partner. You are helpful, enthusiastic, and concise.',
  });
...
export const generateSearchResponse = async (
  ...
  const messages: ChatMessage[] = history.map((h) => ({
    role: h.role === 'model' ? 'assistant' : 'user',
    content: h.parts
      .map((p) => (p.text ? { type: 'text', text: p.text } : null))
      .filter(Boolean) as OpenRouterContentItem[],
  }));
  ...
  messages.unshift({
    ...
    content: [
      {
        type: 'text',
        text: 'You are an expert Trend Researcher for LinkedIn branding. Search the web for the latest trends, data, and visual styles. Be specific, cite sources if possible, and focus on actionable insights for banner design.',
      },
    ],
  });
...
export const generatePromptFromRefImages = async (images: string[], rawUserHint: string) => {
  ...
  const content: OpenRouterContentItem[] = [
    { type: 'text', text: `Analyze these reference images. Hint: ${userHint}. Return a prompt.` },
  ];
