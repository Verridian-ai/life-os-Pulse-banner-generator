...
export const analyzeImageForPrompts = async (base64Image: string) => {
  const content: OpenRouterContentItem[] = [
    ...
    {
      type: 'text',
      text: 'Suggest 3 magic edit prompts and 3 generation prompts. Return JSON: { "magicEdit": [], "generation": [] }',
    },
  ];
...
export const analyzeCanvasAndSuggest = async (
  ...
  _brandProfile: unknown = null,
) => {
  const content: OpenRouterContentItem[] = [
    {
      type: 'image_url',
      image_url: {
        url: canvasScreenshot.startsWith('data:')
          ? canvasScreenshot
          : `data:image/png;base64,${canvasScreenshot}`,
      },
    },
    {
      type: 'text',
      text: 'Analyze this LinkedIn banner. Suggest 3 improvements. Return JSON: { "suggestions": [], "reasoning": "" }',
    },
  ];
...
export const compareImages = async (image1: string, image2: string) => {
  const content: OpenRouterContentItem[] = [
    {
      type: 'text',
      text: 'Compare these two images side-by-side. Analyze differences in composition, colors, brightness, and overall effectiveness for a LinkedIn banner. Return JSON: { "comparison": "detailed comparison text", "preference": "image 1 or image 2", "reasoning": "why" }',
    },
    {
      type: 'image_url',
      image_url: { url: image1.startsWith('data:') ? image1 : `data:image/png;base64,${image1}` },
    },
    {
      type: 'image_url',
      image_url: { url: image2.startsWith('data:') ? image2 : `data:image/png;base64,${image2}` },
    },
  ];
  ...
  const messages: ChatMessage[] = [
    {
      role: 'system',
      content: 'You are Nano Banna Pro, an expert design AI. You compare visual designs.',
    },
    { role: 'user', content: content },
  ];
