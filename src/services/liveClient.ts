...
export class LiveClient {
  ...
  private session: {
    sendRealtimeInput: (input: { audio: { mimeType: string; data: string } }) => Promise<void>;
    close?: () => Promise<void>;
  } | null = null;
  ...
  async connect(
    ...
    this.sessionPromise = this.client.live.connect({
      ...
      config: {
        ...
        systemInstruction:
          'You are a helpful creative director assisting a user in designing their LinkedIn banner. Keep responses concise and focused on design advice, color psychology, and professional branding.',
      },
      callbacks: {
        onopen: () => {
        ...
        onmessage: async (msg: LiveServerMessage) => {
          ...
          const legacyServerContent = msg.serverContent as
    ...
    try {
      this.session = (await this.sessionPromise) as {
        sendRealtimeInput: (input: { audio: { mimeType: string; data: string } }) => Promise<void>;
        close?: () => Promise<void>;
      };
