...
// Supabase configuration
const SUPABASE_URL =
  import.meta.env.VITE_SUPABASE_URL || 'https://bkergrdlytwvdzszmuos.supabase.co';
const SUPABASE_ANON_KEY =
  import.meta.env.VITE_SUPABASE_ANON_KEY || 'sb_publishable_kKxTC0aLBuuqzN5QsaaPnw_Wj8FoSEy';
 ...
 */
export const uploadImage = async (base64Image: string, fileName?: string): Promise<string> => {
  try {
    ...
    const { error } = await supabase.storage.from(IMAGES_BUCKET).upload(filePath, blob, {
    ...
    if (error) {
      ...
      if (networkError.type === 'cors') {
        throw new Error(
          'Supabase CORS error - Check storage bucket permissions and ensure bucket is public.',
        );
...
export const listUserImages = async (): Promise<string[]> => {
  try {
    ...
    const { data, error } = await supabase.storage.from(IMAGES_BUCKET).list(userId, {
      limit: 100,
      offset: 0,
    ...
    const imageUrls = data.map((file) => {
      const {
        data: { publicUrl },
      } = supabase.storage.from(IMAGES_BUCKET).getPublicUrl(`${userId}/${file.name}`);
      return publicUrl;
    });
