...
import { editImage, removeBackground, upscaleImage, outpaintImage, restoreImage } from './imageEditService';
import { api } from './api';
...
vi.mock('./apiKeyStorage', () => ({
    getUserAPIKeys: vi.fn().mockResolvedValue({ replicate_api_key: 'test', openrouter_api_key: 'test' }),
}));
...
describe('imageEditService', () => {
    ...
    describe('editImage', () => {
        it('should attempt OpenRouter first', async () => {
            ...
            expect(api.post).toHaveBeenCalledWith('/api/ai/image/edit', expect.objectContaining({
                prompt: 'modify this',
                provider: 'openrouter',
        ...
        it('should fallback to Replicate if OpenRouter fails', async () => {
            ...
            expect(api.post).toHaveBeenLastCalledWith('/api/ai/image/edit', expect.objectContaining({
                provider: 'replicate'
            }));
    ...
    describe('removeBackground', () => {
        it('should call remove-bg endpoint', async () => {
            ...
            expect(api.post).toHaveBeenCalledWith('/api/ai/image/remove-bg', expect.objectContaining({ image: 'img' }));
    ...
    describe('upscaleImage', () => {
        it('should call upscale endpoint with default scale', async () => {
            ...
            expect(api.post).toHaveBeenCalledWith('/api/ai/image/upscale', expect.objectContaining({ scale: 2 }));
    ...
    describe('outpaintImage', () => {
        it('should call outpaint endpoint with direction', async () => {
            ...
            expect(api.post).toHaveBeenCalledWith('/api/ai/image/outpaint', expect.objectContaining({ direction: 'right' }));
    ...
    describe('restoreImage', () => {
        it('should call restore endpoint with fidelity', async () => {
            ...
            expect(api.post).toHaveBeenCalledWith('/api/ai/image/restore', expect.objectContaining({ fidelity: 0.8 }));
