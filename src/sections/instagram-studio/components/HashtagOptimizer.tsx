...
const hashtagSets: HashtagSet[] = [
  ...
  },
  {
    id: '2',
    name: 'Community',
    hashtags: ['#DesignCommunity', '#CreativeLife'],
    category: 'community',
    avgReach: 50000,
  },
  {
  ...
  return (
    <div className='bg-white/5 backdrop-blur-xl border border-white/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] rounded-2xl p-6'>
      <div className='flex items-center justify-between mb-4'>
        ...
        <span
          className={`text-sm ${selectedHashtags.length >= maxHashtags ? 'text-red-400' : 'text-zinc-400'}`}
        >
              ...
              <div className='flex items-center justify-between mb-2'>
                ...
                {set.avgReach && (
                  <span className='text-xs opacity-70'>
                    ~{(set.avgReach / 1000).toFixed(0)}K reach
                  </span>
                )}
              </div>
              <div className='flex flex-wrap gap-2'>
                {set.hashtags.map((tag) => (
                  <button
                    ...
                    disabled={
                      !selectedHashtags.includes(tag) && selectedHashtags.length >= maxHashtags
                    }
