...
export function XStudioView() {
  const [tweets, setTweets] = useState<ThreadTweet[]>([
    { id: '1', content: 'I\'ve been building AI products for 3 years.\n\nHere are 10 lessons that changed everything:', charCount: 82 },
    { id: '2', content: '1. Ship before you\'re ready\n\nPerfection is the enemy of progress. Your users will tell you what they actually need.', charCount: 113 },
    { id: '3', content: '2. Talk to users every single day\n\nNot surveys. Real conversations. You\'ll be surprised what you learn.', charCount: 98 },
  ]);
  ...
  const updateTweet = (id: string, content: string) => {
    setTweets(tweets.map((t) =>
      t.id === id ? { ...t, content, charCount: content.length } : t
    ));
  };
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-zinc-400 mt-1">
              Build engaging threads and optimize for engagement
            </p>
          </div>

          <div className="flex gap-3">
            <button type="button" className="px-4 py-2 bg-white/5 backdrop-blur-xl border border-white/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] hover:bg-white/10 text-white rounded-xl transition-all">
              Save Draft
            </button>
            <button type="button" className="px-6 py-3 bg-gradient-to-r from-zinc-200 to-white hover:from-white hover:to-zinc-100 text-black font-semibold rounded-xl transition-all shadow-[0_0_40px_rgba(255,255,255,0.2)] hover:shadow-[0_0_50px_rgba(255,255,255,0.3)]">
              Post Thread
            </button>
          </div>
        </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center gap-2">
                          <button type="button" className="w-8 h-8 rounded-lg hover:bg-white/10 flex items-center justify-center text-zinc-400 hover:text-white transition-colors">
                        ...
                        <div className="flex items-center gap-3">
                          <span className={`text-sm ${tweet.charCount > 280 ? 'text-red-400' : 'text-zinc-500'}`}>
                            {tweet.charCount}/280
                          </span>
                          {tweets.length > 1 && (
                            <button
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-[4px_4px_8px_rgba(0,0,0,0.4),-4px_-4px_8px_rgba(255,255,255,0.05),inset_0_1px_1px_rgba(255,255,255,0.1)]">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-3xl font-bold bg-gradient-to-r from-zinc-200 to-white bg-clip-text text-transparent">{engagementScore}</span>
              </div>
            ...
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] rounded-2xl p-6">
              ...
              <div className="space-y-2">
                {['Agree & Expand', 'Add Context', 'Ask Question', 'Disagree', 'Humor', 'Summarize'].map((angle) => (
                  <button
                    key={angle}
                    type="button"
