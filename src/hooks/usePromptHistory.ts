...
export const usePromptHistory = () => {
  ...
  const addPrompt = useCallback(
    (promptText: string, category: string = 'general', tags: string[] = []) => {
      ...
      const existingIndex = loaded.findIndex(
        (p) => p.prompt.toLowerCase() === normalizedText.toLowerCase(),
      );
  ...
  const toggleFavorite = useCallback(
    (id: string) => {
      const loaded = promptHistoryStorage.get<PromptRecord[]>(STORAGE_KEY) || [];
      const updated = loaded.map((p) => (p.id === id ? { ...p, isFavorite: !p.isFavorite } : p));
      saveHistory(updated);
    },
    [saveHistory],
  );
  ...
  const getFrequentPrompts = useCallback(
    (limit = 20) => {
