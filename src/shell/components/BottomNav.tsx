...
export function BottomNav({ items, onNavigate, onCreateNew, className = '' }: BottomNavProps) {
  ...
  return (
    <>
      ...
      <nav
        className={`fixed bottom-0 left-0 right-0 bg-black/80 backdrop-blur-xl border-t border-white/5 z-50 pb-[env(safe-area-inset-bottom)] ${className}`}
      >
          ...
          {leftItems.map((item) => (
            <button
              ...
              className={`flex flex-col items-center gap-1 p-2 transition ${
                item.isActive ? 'text-sky-400' : 'text-zinc-500 hover:text-sky-300'
              }`}
            >
              {item.icon}
