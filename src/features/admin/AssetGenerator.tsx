...
const ProgressBar = ({ progress, total, label }: { progress: number; total: number; label: string }) => (
...
export const AssetGenerator = () => {
    ...
    const handleGenerateAll = async () => {
        if (!window.confirm(`This will generate ${BANNER_TEMPLATES.length} images using your connected AI provider. This may take a while and incur costs. Continue?`)) {
            return;
        }
    ...
    return (
        <div className="min-h-screen bg-black text-white p-8 font-sans">
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="flex items-center justify-between border-b border-white/10 pb-6">
                    <div>
                        ...
                        <p className="text-zinc-400">
                            Batch generate high-fidelity assets for the {BANNER_TEMPLATES.length} template definitions using <span className="text-blue-400 font-bold">Nano Banana Pro</span> engine.
                        </p>
                    </div>
              ...
              `}
                        >
                            {isGenerating ? <RefreshCw className="animate-spin w-5 h-5" /> : <RefreshCw className="w-5 h-5" />}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-zinc-900 border border-white/10 p-6 rounded-2xl">
                        <div className="text-zinc-500 text-xs font-bold uppercase tracking-wider mb-2">Total Templates</div>
                        <div className="text-4xl font-black">{BANNER_TEMPLATES.length}</div>
                    </div>
                    <div className="bg-zinc-900 border border-white/10 p-6 rounded-2xl">
                        <div className="text-green-500 text-xs font-bold uppercase tracking-wider mb-2">Generated</div>
                        <div className="text-4xl font-black text-green-400">{generatedCount}</div>
                    </div>
                    <div className="bg-zinc-900 border border-white/10 p-6 rounded-2xl">
                        <div className="text-red-500 text-xs font-bold uppercase tracking-wider mb-2">Failed</div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="bg-zinc-950 border border-white/10 rounded-2xl overflow-hidden flex flex-col h-[500px]">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 grid grid-cols-2 gap-4">
                            {Array.from(generatedBlobs.entries()).map(([id, blob]) => (
                                <div key={id} className="aspect-video bg-zinc-900 rounded-lg overflow-hidden border border-white/5 relative group">
                                    <img src={URL.createObjectURL(blob)} alt={`Generated asset for template ${id}`} className="w-full h-full object-cover" />
                    ...
                    <div className="bg-zinc-950 border border-white/10 rounded-2xl overflow-hidden flex flex-col h-[500px]">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-2 font-mono text-xs">
                            {logs.map((log, i) => (
                                <div key={i} className="text-zinc-400 border-b border-white/5 pb-1 mb-1 last:border-0">
                                    {log}
                                </div>
                            ))}
                            {logs.length === 0 && (
