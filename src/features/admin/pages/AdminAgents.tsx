...
import { ApiRoutingGraph, AgentMetricsDashboard } from '../components/agents';
import type {
  AgentConfig,
...
function BudgetProgressBar({
  ...
  return (
    <div className='h-2 bg-zinc-800 rounded-full overflow-hidden'>
      <div ref={barRef} className='h-full bg-gradient-to-r from-green-500 to-amber-500' />
    </div>
  );
}
...
const MOCK_DASHBOARD_SUMMARY: AgentDashboardSummary = {
  ...
  apiHealth: [
    ...
    },
    {
      ...
      latencyMs: 50,
      uptime24h: 1.0,
      features: ['chat', 'embeddings', 'realtime'],
    },
    {
      provider: 'cognee',
      status: 'healthy',
...
export function AdminAgents(): React.ReactElement {
  ...
  const agentConnections = agents.map((agent) => ({
    ...
    apis: ['openrouter', 'cognee', 'langfuse'] as Array<
      'openrouter' | 'replicate' | 'openai' | 'cognee' | 'langfuse'
    >,
  }));
                    ...
                    <div className='flex gap-1 mt-4 bg-black/30 rounded-lg p-1 overflow-x-auto'>
                      {tabs.map((tab) => (
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-md transition whitespace-nowrap ${
                            activeTab === tab.id
                  ...
                  {activeTab === 'parameters' && (
                    <div className='p-6 space-y-6'>
                      <div className='bg-blue-500/10 border border-blue-500/30 rounded-xl p-4'>
                        ...
                        <p className='text-zinc-400 text-sm'>
                          Fine-tune the AI model behavior. Changes affect all future calls to this
                          agent.
                        </p>
                      </div>
                      ...
                      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                        ...
                        <div className='space-y-2'>
                          <div className='flex items-center justify-between'>
                            <label
                              htmlFor='param-presence-penalty'
                              className='text-sm font-medium text-zinc-400'
                            >
                              Presence Penalty
                            </label>
                          ...
                          <input
                            ...
                            className='w-full accent-purple-500'
                          />
                          <p className='text-zinc-600 text-xs'>Encourages topic diversity</p>
                        </div>

                        {/* Stop Sequences */}
                        <div className='space-y-2'>
                          <label
                            htmlFor='param-stop-sequences'
                            className='text-sm font-medium text-zinc-400 block'
                          >
                            Stop Sequences
                          </label>
                  ...
                  {activeTab === 'testing' && (
                    <div className='p-6 space-y-6'>
                      <div className='bg-purple-500/10 border border-purple-500/30 rounded-xl p-4'>
                        ...
                        <p className='text-zinc-400 text-sm'>
                          Test your agent with sample prompts. Results show response quality,
                          latency, and cost.
                        </p>
                      </div>
                  ...
                  {activeTab === 'budget' && (
                    <div className='p-6 space-y-6'>
                      <div className='bg-amber-500/10 border border-amber-500/30 rounded-xl p-4'>
                        ...
                        <p className='text-zinc-400 text-sm'>
                          Set spending limits to prevent runaway costs. Alerts are sent when
                      ...
                      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                        <div className='space-y-2'>
                          ...
                          <input
                            ...
                            className='w-full px-4 py-2 bg-black/30 border border-white/10 rounded-lg text-white focus:outline-none focus:border-purple-500'
                          />
                          <p className='text-zinc-600 text-xs'>Leave empty for no daily limit</p>
                        </div>
                        ...
                        <div className='space-y-2'>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEditedBudget((b) => ({
                                ...b,
                                monthlyLimitUsd: e.target.value ? parseFloat(e.target.value) : null,
                              }))
                            }
                            placeholder='No limit'
                            min='0'
                            ...
                            className='w-full px-4 py-2 bg-black/30 border border-white/10 rounded-lg text-white focus:outline-none focus:border-purple-500'
                          />
                          <p className='text-zinc-600 text-xs'>Leave empty for no monthly limit</p>
                        </div>

                        <div className='space-y-2 md:col-span-2'>
                          <div className='flex items-center justify-between'>
                            <label
                              htmlFor='budget-alert-threshold'
                              className='text-sm font-medium text-zinc-400'
                            >
                              Alert Threshold
                            </label>
                  ...
                  {activeTab === 'context' && (
                    <div className='p-6 space-y-6'>
                      <div className='bg-blue-500/10 border border-blue-500/30 rounded-xl p-4'>
                        ...
                        <p className='text-zinc-400 text-sm'>
                          Documents added here are processed by Cognee RAG and injected into the
                          agent's context window during execution.
                        </p>
                      </div>
                  ...
                  {activeTab === 'history' && (
                    <div className='p-6 space-y-6'>
                      <div className='bg-zinc-500/10 border border-zinc-500/30 rounded-xl p-4'>
                        ...
                        <p className='text-zinc-400 text-sm'>
                          View and restore previous configurations. Click "Restore" to load a
                          previous version into the editor.
                        </p>
                      </div>
                        ...
                        {versionHistory.length === 0 ? (
                          ...
                          versionHistory.map((version, index) => (
                            <div
                              key={version.id}
                              className={`bg-black/30 border rounded-xl p-4 ${
                                index === 0 ? 'border-green-500/30' : 'border-white/5'
                              }`}
                            >
                              <div className='flex items-start justify-between mb-3'>
                                <div className='flex items-center gap-3'>
                                  <div
                                    className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                                      index === 0 ? 'bg-green-500/20' : 'bg-zinc-800'
                                    }`}
                                  >
                                    <span
