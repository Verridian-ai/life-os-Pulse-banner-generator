          ...
          {isLoading ? (
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-pulse'>
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className='bg-zinc-900/50 border border-white/5 rounded-xl p-6 h-32'
                ></div>
              ))}
            </div>
          ) : (
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
                ...
                <div className='bg-zinc-900/50 border border-white/5 rounded-xl p-6'>
                  ...
                  <p className='text-3xl font-bold text-white'>
                    {billingMetrics.totalActiveSubscribers}
                  </p>
                ...
                <div className='bg-zinc-900/50 border border-white/5 rounded-xl p-6'>
                  ...
                  <p className='text-3xl font-bold text-white'>
                    {formatUSD(billingMetrics.monthlyRevenue)}
                  </p>
                  <p className='text-zinc-500 text-xs mt-1'>
              ...
              {billingMetrics.dailyRevenue.length > 0 && (
                <div className='bg-zinc-900/50 border border-white/5 rounded-2xl p-6 mt-4'>
                  ...
                  <div className='flex items-end gap-1 h-32'>
                    {(() => {
                      const maxRevenue = Math.max(
                        ...billingMetrics.dailyRevenue.map((d) => d.revenue),
                      );
                  ...
                  <div className='flex justify-between mt-2 text-xs text-zinc-600'>
                    ...
                    <span>
                      {billingMetrics.dailyRevenue[billingMetrics.dailyRevenue.length - 1]?.date}
                    </span>
                  </div>
                </div>
              )}
                  ...
                  {isLoading ? (
                    ...
                    transactions.map((tx) => (
                      <tr key={tx.id} className='hover:bg-white/5 transition'>
                        ...
                        <td className='p-4 text-white font-medium'>{tx.userEmail}</td>
                        <td className='p-4'>
                          <span
