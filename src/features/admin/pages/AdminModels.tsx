...
function ModelCard({
  ...
  return (
    <div
      className={`bg-zinc-900 border ${providerStyle.border} rounded-xl p-5 hover:bg-zinc-800/50 transition`}
    >
      {/* Header */}
      <div className='flex items-start justify-between mb-4'>
        <div className='flex items-center gap-3'>
          ...
          <div>
            ...
            <p className='text-zinc-500 text-xs font-mono truncate max-w-[200px]'>
              {model.modelId}
            </p>
          </div>
        </div>
        <span
          className={`px-2 py-1 rounded text-xs font-medium ${providerStyle.bg} ${providerStyle.text}`}
        >
          {model.provider}
        </span>
      </div>
      ...
      <div className='grid grid-cols-2 gap-3'>
        ...
        <div className='bg-black/30 rounded-lg p-3'>
          ...
          <p
            className={`font-semibold ${model.successRate >= 95 ? 'text-green-400' : model.successRate >= 80 ? 'text-yellow-400' : 'text-red-400'}`}
          >
      ...
      {model.lastUsedAt && (
        <p className='text-zinc-600 text-xs mt-3'>
          Last used:{' '}
          {new Date(model.lastUsedAt).toLocaleString(undefined, {
            month: 'short',
            day: 'numeric',
            hour: '2-digit',
            minute: '2-digit',
...
function ComparisonTable({
  ...
  if (rankedModels.length === 0) {
    return <div className='text-center py-8 text-zinc-500'>No models to compare</div>;
  }
              ...
              <tr key={model.modelId} className='hover:bg-white/5 transition'>
                ...
                <td className='px-4 py-3'>
                  <div>
                    ...
                    <p className='text-zinc-500 text-xs font-mono truncate max-w-[180px]'>
                      {model.modelId}
                    </p>
                  </div>
                </td>
