...
export const LinkedInImageGenerator: React.FC<LinkedInImageGeneratorProps> = ({
  ...
  const [style, setStyle] = useState<'professional' | 'creative' | 'bold' | 'minimalist'>(
    'professional',
  );
  ...
  const handleGenerate = () => {
    ...
    const generateRequest: ImageGenerationRequest = {
      prompt:
        customPrompt ||
        `A professional LinkedIn featured image about ${topic}, ${style} style, high quality, 4k.`,
      dimensions,
      style,
    };
        ...
        <div className='space-y-3'>
          ...
          <div className='flex gap-2'>
            {(
              ...
              >
                <span
                  className={`material-icons text-sm ${a.id === 'portrait' ? 'rotate-90' : ''}`}
                >
                  {a.icon}
                </span>
