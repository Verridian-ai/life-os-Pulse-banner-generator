...
export const validatePrompt = (prompt: string, maxLength = MAX_PROMPT_LENGTH): string => {
    ...
    if (sanitized.length > maxLength) {
        console.warn(`[Validation] Prompt length ${sanitized.length} exceeds limit ${maxLength}. Truncating.`);
 ...
 */
export const validateChatHistory = (history: { role: string; parts: { text?: string }[] }[]): void => {
    let totalLength = 0;
    ...
    if (totalLength > MAX_CONTEXT_LENGTH) {
        throw new ValidationError(`Chat history exceeds maximum allowed context length of ${MAX_CONTEXT_LENGTH} characters.`);
    }
};
