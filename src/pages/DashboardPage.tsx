...
import { PlatformCard, PlatformType, RecentDesigns, TemplatesGrid, BrandKitPanel } from '../components/dashboard';
...
export function DashboardPage({ onEnterStudio, onOpenPreferences, onOpenAuth, onSelectTemplate, onCreateBrand }: DashboardPageProps) {
  ...
  const navigationItems: NavigationItem[] = [
    ...
    { id: 'design-studio', label: 'Design Studio', icon: <PenTool className="w-5 h-5" />, onClick: () => onEnterStudio('linkedin') },
  ...
  const SectionHeader = ({ icon: Icon, title, description, iconColor = 'text-yellow-400' }: {
  ...
  const ComingSoon = ({ title, description, launchDate = 'Q1 2026' }: { title: string; description: string; launchDate?: string }) => (
      ...
      >
            ...
            <div className="relative">
              ...
              <div className="relative z-10">
                <h1 className="text-2xl md:text-3xl font-bold text-white mb-2 tracking-tight">
                  {user ? (
                    <>Welcome back, <span className="bg-gradient-to-r from-yellow-300 to-yellow-400 bg-clip-text text-transparent">{user.first_name || user.full_name?.split(' ')[0] || 'Creator'}</span></>
                  ) : (
                    <>Welcome to <span className="bg-gradient-to-r from-yellow-300 to-yellow-400 bg-clip-text text-transparent">Signal</span></>
                  )}
                </h1>
        ...
        {activeView === 'quick-generate' && (
          <div className="space-y-6">
            <SectionHeader icon={Zap} title="Quick Generate" description="Create designs in one click" />
        ...
        {activeView === 'templates' && (
          <div className="space-y-6">
            <SectionHeader icon={Layout} title="Templates" description="Browse and use pre-designed templates" />
            <TemplatesGrid
        ...
        {activeView === 'gallery' && (
          <div className="space-y-6">
            <SectionHeader icon={Image} title="Gallery" description="View all your generated images" />
            <Suspense fallback={<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">{[...Array(8)].map((_, i) => <Skeleton key={i} height={200} />)}</div>}>
        ...
        {activeView === 'projects' && (
          <div className="space-y-6">
            <SectionHeader icon={FolderOpen} title="Projects" description="Organize your designs into projects" />
            <ComingSoon
        ...
        {activeView === 'brand' && (
          <div className="space-y-6">
            <SectionHeader icon={Palette} title="Brand Kit" description="Manage your brand colors, fonts, and assets" />
            <BrandKitPanel
              onCreateBrand={() => {
                if (onCreateBrand) {
                  onCreateBrand();
        ...
        {activeView === 'chat-brainstorm' && (
          <div className="space-y-6 h-full">
            <SectionHeader icon={MessageSquare} title="AI Chat" description="Brainstorm ideas with AI" />
