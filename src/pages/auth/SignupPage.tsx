/**
 * SignupPage - Redirects to WorkOS AuthKit for sign up
 *
 * Shows a brief loading state with Nanobanna branding,
 * then redirects to the AuthKit hosted signup UI with screen_hint=sign-up.
 */
...
export function SignupPage(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    // Once auth state is known, redirect to AuthKit with signup hint
    if (!isLoading && !isAuthenticated) {
      // Redirect to backend which initiates WorkOS AuthKit flow
      // screen_hint=sign-up tells AuthKit to show the signup form
      window.location.href = '/api/auth/workos/authorize?screen_hint=sign-up';
    }
    ...
    <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center">
      ...
      <div className="relative z-10 text-center">
        ...
        <p className="text-zinc-400 text-sm max-w-xs mx-auto">
          You'll be redirected to our secure signup page in a moment...
        </p>

        {/* Fallback link */}
        <div className="mt-8">
          <a
            href="/api/auth/workos/authorize?screen_hint=sign-up"
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 text-center">
        ...
        <a
          href="/"
          className="text-zinc-500 hover:text-zinc-300 text-sm transition-colors flex items-center justify-center gap-2"
        >
