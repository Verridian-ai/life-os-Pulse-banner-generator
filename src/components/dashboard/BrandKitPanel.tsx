...
import { Plus, Check, Trash2, Upload, Palette, MoreVertical, Sparkles, ArrowRight, Type, Droplets } from 'lucide-react';
  ...
  return (
    <div className="bg-zinc-900/80 border border-white/5 rounded-2xl p-6 overflow-hidden">
      ...
      <div className="flex gap-2 mb-5">
        {[1, 2, 3, 4].map((j) => (
          <div key={j} className={`w-9 h-9 bg-zinc-800/50 rounded-full animate-pulse ${j === 1 ? 'delay-100' : j === 2 ? 'delay-200' : j === 3 ? 'delay-300' : j === 4 ? 'delay-[400ms]' : ''}`} />
        ))}
      </div>
          ...
          {brandProfiles.map((profile) => {
            ...
            return (
              ...
              >
                ...
                {/* Menu */}
                <div className="absolute top-4 right-4" ref={menuOpenId === profile.id ? menuRef : undefined}>
                  {!isActive && (
                ...
                {/* Brand Name */}
                <h3 className="font-bold text-white text-lg mb-5 pr-16 group-hover:text-sky-100 transition-colors">{profile.name}</h3>
                ...
                <div className="flex gap-2 mb-5">
                  ...
                  {(profile.colors?.length || 0) > 5 && (
                    <div className="w-9 h-9 rounded-full bg-zinc-800/80 border-2 border-zinc-700 flex items-center justify-center">
                      <span className="text-[10px] text-zinc-400 font-medium">+{profile.colors!.length - 5}</span>
                    </div>
                  )}
