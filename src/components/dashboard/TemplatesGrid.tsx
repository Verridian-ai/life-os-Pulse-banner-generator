...
function TemplateCard({ template, onSelect }: { template: BannerTemplate; onSelect: () => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Info */}
      <div className='p-4'>
        <h3 className='font-bold text-white text-sm mb-1.5 truncate group-hover:text-sky-100 transition-colors'>
          {template.title}
        </h3>
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className='flex flex-col md:flex-row md:items-end justify-between gap-4'>
        <div>
          ...
          <p className='text-zinc-400 text-sm'>
            <span className='text-sky-400 font-semibold'>{BANNER_TEMPLATES.length}</span>{' '}
            professional templates to jumpstart your designs
          </p>
        </div>
      </div>
      ...
      {searchQuery || selectedIndustry !== 'All' ? (
        <div className='flex items-center gap-2 text-sm text-zinc-400'>
          <span>
            Showing <span className='text-white font-medium'>{filteredTemplates.length}</span>{' '}
            templates
          </span>
          {(searchQuery || selectedIndustry !== 'All') && (
            <button
