...
const BannerCanvas = forwardRef<BannerCanvasHandle, BannerCanvasProps>(({
    ...
    const elementRects = useRef<Record<string, { x: number, y: number, w: number, h: number, rotation: number }>>({});
    ...
    const renderCanvas = (ctx: CanvasRenderingContext2D, includeSafeZones: boolean, includeHandles: boolean) => {
        // Clear
    ...
    const drawElementsAndOverlays = (ctx: CanvasRenderingContext2D, drawGuides: boolean, drawHandles: boolean) => {
        // 1. Draw User Elements
        elements.forEach(el => {
            const rotation = el.rotation || 0;
            const angleRad = (rotation * Math.PI) / 180;

            let width = 0, height = 0, rectX = el.x;
    ...
    const isOverHandle = (x: number, y: number, rect: { x: number, y: number, w: number, h: number, rotation: number }): ResizeHandle | null => {
        const cx = rect.x + rect.w / 2;
        const cy = rect.y + rect.h / 2;
        ...
        for (const h of handles) {
            if (localP.x >= h.cx - HANDLE_SIZE && localP.x <= h.cx + HANDLE_SIZE &&
                localP.y >= h.cy - HANDLE_SIZE && localP.y <= h.cy + HANDLE_SIZE) {
                return h.t;
            }
        }
        ...
        if (localP.x >= rotCx - HANDLE_SIZE && localP.x <= rotCx + HANDLE_SIZE &&
            localP.y >= rotCy - HANDLE_SIZE && localP.y <= rotCy + HANDLE_SIZE) {
            return 'rot';
        }
        return null;
    };
    ...
    const handleMouseDown = (e: React.MouseEvent | React.TouchEvent) => {
        ...
        for (const id of reversedIds) {
            ...
            if (localP.x >= rect.x && localP.x <= rect.x + rect.w && localP.y >= rect.y && localP.y <= rect.y + rect.h) {
                hitId = id;
                break;
            }
        }
    ...
    const handleMouseMove = (e: React.MouseEvent | React.TouchEvent) => {
        ...
        if (dragState) {
            ...
            if (dragState.mode === 'move') {
                ...
                if (el.type === 'text') {
                    let scale = 1;
                    if (dragState.handle === 'n' || dragState.handle === 's') scale = newH / dragState.initialH;
    ...
    const [profileDrag, setProfileDrag] = useState<{ startX: number, startY: number, startPX: number, startPY: number } | null>(null);
                ...
                >
                    {profilePic ? (
                        ...
                        <div className="text-slate-400 font-bold text-center leading-tight select-none pointer-events-none">
                            <span className="material-icons text-4xl block md:text-5xl lg:text-6xl mb-1">person</span>
                            <span className="text-[8px] md:text-[10px] lg:text-sm whitespace-nowrap px-2">524 px zone</span>
                            <div className="mt-2 text-[10px] text-slate-300 opacity-60">
                                Ctrl+Scroll to Zoom<br />Drag to Move
                            </div>
                        </div>
                    )}
                </div>
