...
const AssetsPanel: React.FC = () => {
    const {
        refImages, setBgImage, handleRefUpload: onRefUpload,
        profilePic, handleProfileUpload: onProfileUpload, isProcessingImg, addElement,
    ...
    return (
        <div className="bg-zinc-900/40 backdrop-blur-md p-6 rounded-3xl border border-white/10 shadow-xl flex flex-col relative group">
            ...
            {isProcessingImg && <div className="absolute inset-0 bg-black/60 z-20 rounded-3xl flex items-center justify-center text-xs text-white font-bold backdrop-blur-sm"><span className="animate-pulse">OPTIMIZING...</span></div>}
            ...
            <div className="flex items-center justify-between mb-6 relative z-10">
                ...
                <span className="text-[9px] font-bold uppercase tracking-widest bg-white/10 border border-white/5 px-2 py-1 rounded-md text-zinc-400">Uploads</span>
            </div>

            <div className="space-y-6 relative z-10 flex-1">
                {/* Background Upload Section */}
                <div>
                    <label className="block text-[10px] font-bold text-zinc-500 mb-3 uppercase tracking-widest">Canvas Background</label>
                    <div className="flex gap-2">
                        <label className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl cursor-pointer transition ${BTN_NEU_SOLID} group/bg`}>
                            <span className="material-icons text-zinc-400 group-hover/bg:text-white transition">add_photo_alternate</span>
                ...
                <div>
                    <label className="block text-[10px] font-bold text-zinc-500 mb-3 uppercase tracking-widest">Logos</label>
                    <div className="grid grid-cols-4 gap-2">
                        {refImages.map((img: string, idx: number) => (
                            <div key={idx} className="relative group/item aspect-square rounded-xl overflow-hidden border border-white/10 bg-black/50">
                ...
                <div className="pt-4 border-t border-white/10 mb-4">
                    <label className="block text-[10px] font-bold text-zinc-500 mb-3 uppercase tracking-widest">Logos & Image Overlays</label>
                    <div className="grid grid-cols-4 gap-2">
                        {/* Reference Images List */}
                        {refImages.map((img, i) => (
                            <div key={i} className="aspect-square bg-zinc-800 rounded-lg relative group overflow-hidden border border-zinc-700/50">
                                <img src={img} alt="ref" className="w-full h-full object-contain p-2 opacity-70 group-hover:opacity-100 transition" />
                                <div className="absolute inset-0 bg-black/80 flex items-center justify-center opacity-0 group-hover:opacity-100 transition backdrop-blur-sm cursor-pointer"
                                    onClick={() => addElement({
                                        id: Date.now().toString(),
                                        type: 'image',
                                        content: img,
                ...
                <div className="pt-4 border-t border-white/10">
                    <label className="block text-[10px] font-bold text-zinc-500 mb-3 uppercase tracking-widest">Profile Overlay</label>
                        ...
                        <div className="flex items-center gap-4">
                            <div className="w-14 h-14 shrink-0 aspect-square rounded-full bg-zinc-800 overflow-hidden border-2 border-zinc-700 relative group/profile shadow-inner">
                                {profilePic ? (
                                    ...
                                    <span className="material-icons text-zinc-600 absolute inset-0 flex items-center justify-center">person</span>
                                )}
