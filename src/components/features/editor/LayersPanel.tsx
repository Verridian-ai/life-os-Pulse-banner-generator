    ...
    <div className='bg-zinc-900/40 backdrop-blur-md p-6 rounded-3xl border border-white/10 shadow-xl flex flex-col relative group'>
      ...
      <div className='flex items-center justify-between mb-6 relative z-10'>
        ...
        <button
          ...
          className={`h-8 px-4 text-[9px] rounded-full ${BTN_NEU_SOLID}`}
        >
          + Add Text
        </button>
      </div>
                  ...
                  >
                    {FONT_OPTIONS.map((font) => (
                      <option
                        key={font}
                        value={font}
                        className={`bg-zinc-900 text-zinc-300 font-[${font.replace(/\s/g, '_')}]`}
                      >
                        {font}
                      </option>
                    ))}
                ...
                <div className='col-span-2 flex bg-white/5 rounded-lg p-0.5'>
                  {(['left', 'center', 'right'] as const).map((align) => (
                    <button
                      ...
                      className={`flex-1 py-1 rounded-md transition-all flex items-center justify-center ${
