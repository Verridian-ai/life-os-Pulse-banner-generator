...
export const ProfileEditorModal: React.FC<ProfileEditorModalProps> = ({
  ...
  const handleEnhance = async () => {
    ...
    try {
      ...
      const service = await import('../../../services/replicate').then((m) =>
        m.getReplicateService(),
      );
      // Face Enhance
        ...
        <div className='flex-1 bg-black/50 p-8 flex items-center justify-center relative'>
          ...
          {isProcessing && (
            <div className='absolute inset-0 flex flex-col items-center justify-center z-20'>
              ...
              <div className='text-white font-bold animate-pulse text-sm uppercase tracking-widest'>
                {statusMsg}
              </div>
            </div>
          )}
          ...
          <div className='absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-4 bg-zinc-900/90 p-2 rounded-xl border border-white/10'>
            <button
              ...
              className='p-2 hover:bg-white/10 rounded-lg text-white material-icons'
            >
              remove
            </button>
        ...
        <div className='w-[350px] bg-zinc-900 border-l border-white/10 flex flex-col'>
          <div className='p-6 border-b border-white/10 flex justify-between items-center'>
            ...
            <button
              type='button'
              onClick={onClose}
              className='text-zinc-500 hover:text-white material-icons'
            >
              close
            </button>
          </div>
