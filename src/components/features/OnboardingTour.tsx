...
export const OnboardingTour: React.FC = () => {
  ...
  if (targetRect) {
    ...
    const popoverStyle: React.CSSProperties = {
      ...
      left: Math.max(16, Math.min(window.innerWidth - POPOVER_WIDTH - 16, targetRect.left + (targetRect.width / 2) - (POPOVER_WIDTH / 2))),
      ...(isTopPosition
          ...
          <div className='flex gap-4'>
            <div className='flex-shrink-0'>
              <div className='inline-flex items-center justify-center w-12 h-12 bg-gradient-to-br from-purple-600 to-pink-600 rounded-xl shadow-lg shadow-purple-500/20'>
                <span className='material-icons text-2xl text-white' aria-hidden="true">{step.icon}</span>
              </div>
            </div>
            <div>
              <h2 id="tour-title" className='text-lg font-black text-white uppercase tracking-wider mb-2'>{step.title}</h2>
              <p className='text-sm text-zinc-400 leading-relaxed mb-4'>{step.content}</p>

              <div className='flex items-center justify-between gap-4'>
                <div className='flex gap-1' role="progressbar" aria-valuenow={currentStep + 1} aria-valuemin={1} aria-valuemax={TOUR_STEPS.length} aria-label={`Step ${currentStep + 1} of ${TOUR_STEPS.length}`}>
                  {TOUR_STEPS.map((_, i) => (
                    <div key={i} className={`h-1.5 rounded-full transition-all ${i === currentStep ? 'w-6 bg-purple-500' : 'w-1.5 bg-zinc-800'}`} />
                  ))}
                </div>
      ...
      >
        ...
        <div className='text-center mb-6'>
          <div className='inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-purple-600 to-pink-600 rounded-2xl mb-4 shadow-lg shadow-purple-500/20'>
            <span className='material-icons text-3xl text-white' aria-hidden="true">{step.icon}</span>
          </div>
          <h2 id="tour-title" className='text-2xl font-black text-white uppercase tracking-wider mb-2'>{step.title}</h2>
