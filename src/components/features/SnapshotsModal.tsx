...
export const SnapshotsModal: React.FC<SnapshotsModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        ...
        >
            <div ref={modalRef} className='bg-zinc-900 border border-white/10 rounded-3xl p-6 w-full max-w-md max-h-[80vh] overflow-y-auto relative shadow-2xl'>
                {/* Header */}
                <div className="flex justify-between items-center mb-6">
                    <h2 id="snapshots-modal-title" className='text-xl font-bold text-white flex items-center gap-2'>
                ...
                <div className="space-y-3">
                    {snapshots.length === 0 ? (
                        ...
                        snapshots.map(snapshot => (
                            <div key={snapshot.id} className="p-4 bg-zinc-950 border border-white/5 rounded-xl flex justify-between items-center group hover:border-purple-500/30 transition">
                                <div className="overflow-hidden mr-4">
                                    <h3 className="text-white font-medium truncate" title={snapshot.name}>{snapshot.name}</h3>
                                    <p className="text-xs text-zinc-500">{new Date(snapshot.createdAt).toLocaleString()}</p>
                                </div>
