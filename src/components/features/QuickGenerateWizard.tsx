...
export const QuickGenerateWizard: React.FC<QuickGenerateWizardProps> = ({ onClose, onComplete }) => {
  ...
  const generateVariants = async (finalAnswers: Record<string, string>) => {
    ...
    try {
      ...
      const results = await Promise.all([
        generateImage(`${basePrompt}, variant 1`, [], '1K', { width: BANNER_WIDTH, height: BANNER_HEIGHT }),
        generateImage(`${basePrompt}, variant 2, different colors`, [], '1K', { width: BANNER_WIDTH, height: BANNER_HEIGHT }),
        generateImage(`${basePrompt}, variant 3, alternative layout`, [], '1K', { width: BANNER_WIDTH, height: BANNER_HEIGHT }),
        generateImage(`${basePrompt}, variant 4, minimalist approach`, [], '1K', { width: BANNER_WIDTH, height: BANNER_HEIGHT }),
      ]);
        ...
        {step < QUESTIONS.length ? (
          <div className='animate-fadeIn'>
            <div className='text-center mb-8'>
              ...
              <h2 className='text-2xl font-black text-white uppercase tracking-wider'>{QUESTIONS[step].question}</h2>
            </div>
          ...
          <div className='text-center py-20 animate-pulse'>
            ...
            <h2 className='text-xl font-black text-white uppercase tracking-widest mb-2'>Generating 4 Variants</h2>
            <p className='text-zinc-500 text-sm'>Our AI is crafting your professional designs...</p>
          </div>
        ) : (
          <div className='animate-fadeIn'>
            <div className='text-center mb-8'>
              <h2 className='text-2xl font-black text-white uppercase tracking-wider mb-2'>Your Custom Variants</h2>
              <p className='text-zinc-500 text-sm'>Pick your favorite to start editing or export immediately.</p>
            </div>
              ...
              {variants.map((v, i) => (
                ...
                >
                  ...
                  <div className='absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center'>
                    <span className='bg-white text-black px-4 py-2 rounded-full font-bold text-xs uppercase tracking-widest shadow-lg'>Select</span>
                  </div>
                </div>
              ))}
            </div>
