...
export interface BottomNavProps {
  // Legacy interface (used by App.tsx)
  activeTabId?: string;
  onTabSelect?: (tabId: string) => void;
  showAdminTab?: boolean;
  // New items-based interface
...
export function BottomNav({
  ...
  // New props
  items: providedItems,
  activeItemId: providedActiveId,
  onNavigate: providedOnNavigate,
  onCreateNew,
  className = '',
}: BottomNavProps) {
  // Determine which interface is being used
  const activeItemId = providedActiveId ?? activeTabId ?? '';
  const onNavigate = providedOnNavigate ?? onTabSelect ?? (() => {});
  ...
  // Default items for legacy interface
  const defaultItems = React.useMemo((): NavigationItem[] => {
    const baseItems: NavigationItem[] = [
      {
        id: 'dashboard',
        label: 'Home',
        href: '/dashboard',
        icon: <span className='material-icons text-xl'>home</span>,
      },
      {
        id: 'templates',
        label: 'Templates',
        href: '/templates',
        icon: <span className='material-icons text-xl'>grid_view</span>,
      },
      {
        id: 'design-studio',
        label: 'Studio',
        href: '/studio',
        icon: <span className='material-icons text-xl'>brush</span>,
      },
      {
        id: 'gallery',
        label: 'Gallery',
        href: '/gallery',
        icon: <span className='material-icons text-xl'>photo_library</span>,
      },
    ];
    if (showAdminTab) {
      baseItems.push({
        ...
        label: 'Admin',
        href: '/admin',
        icon: <span className='material-icons text-xl'>admin_panel_settings</span>,
      });
    }
    return baseItems;
  }, [showAdminTab]);
  ...
  const renderNavItem = (item: NavigationItem) => {
    ...
    return (
      <button
        ...
        className={`relative flex flex-col items-center justify-center gap-0.5 p-2 min-w-[72px] min-h-[56px] transition-transform duration-200 ease-[cubic-bezier(0.33,1,0.68,1)] touch-manipulation ${
          isPressed ? 'scale-[0.92]' : 'scale-100'
        }`}
      >
        {/* Active indicator - animated pill */}
        <div
          className={`absolute top-1 left-1/2 -translate-x-1/2 h-1 rounded-full bg-gradient-to-r from-yellow-400 to-yellow-300 transition-all duration-300 ease-[cubic-bezier(0.33,1,0.68,1)] ${
            isActive
        ...
        <span
          className={`transition-all duration-200 ease-[cubic-bezier(0.33,1,0.68,1)] ${
            isActive
        ...
        <span
          className={`text-[10px] font-semibold tracking-wide uppercase transition-all duration-200 ${
            isActive ? 'text-yellow-400 opacity-100' : 'text-stone-500 opacity-80'
          }`}
        >
          {item.label}
        </span>
  ...
  return (
    <>
      ...
      {showFab && (
        ...
        >
          {/* Inner glow effect */}
          <div className='absolute inset-0 rounded-full opacity-50 bg-[radial-gradient(circle_at_30%_30%,rgba(255,255,255,0.3)_0%,transparent_60%)]' />

          {/* Icon */}
          <Plus className='w-6 h-6 text-white relative z-10 drop-shadow-[0_1px_2px_rgba(0,0,0,0.3)]' />
        </button>
      )}
    </>
  );
