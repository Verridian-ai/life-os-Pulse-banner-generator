    ...
    >
      <div className='bg-zinc-900/90 backdrop-blur-xl border border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl relative animate-fadeIn'>
        ...
        <div className='flex flex-col items-center text-center gap-4'>
          <div
            className={`w-12 h-12 rounded-full flex items-center justify-center ${isDestructive ? 'bg-red-500/10 text-red-500' : 'bg-sky-500/10 text-sky-500'}`}
          >
            <span className='material-icons text-2xl'>{isDestructive ? 'warning' : 'info'}</span>
          </div>

          <div>
            <h3 id='confirmation-modal-title' className='text-lg font-bold text-white mb-2'>
              {title}
            </h3>
            <p
          ...
          <div className='flex gap-3 w-full mt-2'>
            ...
            <button
              ...
              className={`flex-1 py-3 px-4 rounded-xl text-white transition font-bold text-xs uppercase tracking-wider shadow-lg ${
                isDestructive
