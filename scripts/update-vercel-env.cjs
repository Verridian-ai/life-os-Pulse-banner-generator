...
ENV_CONTENT.split('\n').forEach(line => {
    ...
    if (line && !line.startsWith('#')) {
        ...
        if (key && valueParts.length > 0) {
            ...
            // Filter only VITE_ keys or relevant ones, and exclude Vercel_API_key, AND skip empty values
            if (key.startsWith('VITE_') && key !== 'Vercel_API_key' && value.length > 0) {
                ENV_VARS[key] = value;
            }
        }
    }
...
async function main() {
    ...
    console.log(Object.keys(ENV_VARS)); // Log keys only for security
